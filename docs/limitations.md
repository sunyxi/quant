...
- The kabu Station mapper, official request contract helpers, fake-transport token client, fake-transport sendorder client, and fake-transport cancelorder client are local-only and do not perform real authentication, send real HTTP requests, cancel orders, query order status, or place live orders.
