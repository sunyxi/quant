...
- The kabu Station mapper, official request contract helpers, fake-transport token client, fake-transport sendorder client, fake-transport cancelorder client, and fake-transport read-only client are local-only and do not perform real authentication, send real HTTP requests, cancel orders, query real order status, query real positions, or place live orders.
