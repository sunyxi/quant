...
- ISSUE-040 performs one point-in-time read-only order-list query and matches only the exact client-order remark. It does not persist evidence, inspect fills, reconstruct order history, reconcile cash or positions, map broker status, cancel duplicates, or integrate with OMS/ledger. `absent` cannot prove that an earlier submission did not occur, and no result authorizes automatic resubmission.
