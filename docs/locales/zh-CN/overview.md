...
首期范围仍是日股、只做多、不隔夜，并在任何实盘前先通过 Shadow Mode。现优先在 macOS 上使用 Moomoo OpenAPI 完成美股能力的脱敏只读 PoC，不下实盘订单。Moomoo JP 当前不支持日股现货实盘 API 交易，因此 kabu Station 仍作为日股未来通道，IBKR 作为美股备选。下一个只读边界由 ISSUE-035 实现。
