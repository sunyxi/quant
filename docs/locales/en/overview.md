...
The localhost boundary now reapplies loopback and read-only policy checks to redirects, rejects encoded endpoint paths, preserves typed status errors for empty HTTP error bodies, and reports configuration, connection, and timeout failures without claiming false connection success.
