...
localhost 境界は、リダイレクトにも loopback と参照専用ポリシーを再適用し、エンコードされた endpoint path を拒否します。空の HTTP エラー本文でも status 別エラーを保持し、configuration、connection、timeout を誤って接続成功と報告しません。
