...
ISSUE-037 は、準備完了した米国株買い指値 intent 向けのオフライン `moomoo-paper-order-dry-run` 契約を追加します。`SIMULATE`、`NORMAL`、`DAY`、`RTH` の固定値と数量・想定元本上限を出力します。SDK の import、口座選択、発注は行わず、デモ取引や実取引を承認しません。
