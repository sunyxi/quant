...
初期リリースの範囲は、日本株、買いのみ、持ち越しなし、実取引前の Shadow Mode のままです。macOS 上の Moomoo OpenAPI を、米国株機能のサニタイズ済み参照専用 PoC として最優先し、実注文は行いません。Moomoo JP は現在、日本株現物の実取引 API に対応していないため、kabu Station を将来の日本株接続先とし、IBKR を米国株の代替候補とします。次の参照専用境界は ISSUE-035 で実装します。
