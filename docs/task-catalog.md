...
- Roadmap: see `docs/roadmap.md#phase-2`
- Summary: Add bounded nested ORB parameter tuning that freezes selection before outer tests, rejects fragile candidates, and exploratorily evaluates signal timing, breakout close location, and VWAP slope filters without treating reused dates as independent evidence.

### Acceptance Criteria

- A deterministic bounded grid contains exactly 192 unique long-only ORB parameter combinations: 96 original combinations and 96 with a 90-minute signal cutoff, 0.7 breakout close-location threshold, and same-direction VWAP slope; all share one side-cost assumption and the engine rejects empty, duplicate, mixed-cost, or oversized candidate sets.
...
- The historical-orb-backtest CLI requires --run with --tune, exposes inner-window and robustness thresholds, emits schema version 4 tuning reports through the existing atomic create-only writer, and preserves schema version 2 for non-tuning runs.
...
- Tests cover candidate bounds, signal cutoff, breakout close location, VWAP slope, inner/outer isolation, cost and concentration rejection, parameter-neighbor stability, frozen aggregate OOS trades, no-go decisions, zero-trade date Sharpe, and CLI schema behavior.
- The preserved-cache result is documented as exploratory and no-go: 1 of 4 outer folds selected a structure-filtered candidate, while nine frozen outer-test trades produced negative mean net basis points, Sharpe, and total PnL.
...
Rollback: Disable historical-orb-backtest --tune, preserve schema version 3 and 4 research evidence, and revert ISSUE-044 through a reviewed PR; the exploratory structure filters can be removed independently by restoring the 96-candidate bound and schema version 3, and no broker order, account, or position side effect exists.
