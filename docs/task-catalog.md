...
- Roadmap: see `docs/roadmap.md#phase-2`
- Summary: Add a broker-independent historical ORB research engine that reuses verified local bar caches and reports complete trade lifecycle, net PnL, cost sensitivity, symbol attribution, and frozen walk-forward evidence.
...
- The loader accepts only an adjacent non-symlink gzip CSV artifact after exact schema, filename, format, columns, row count, date range, symbol set, and SHA-256 validation; it performs no download or broker call.
...
- When one bar touches stop and target, the simulator applies stop first; explicit round-trip costs are deducted from gross PnL and normalized return.
...
- The baseline result includes per-symbol attribution and zero, baseline, and doubled cost scenarios.
- Rolling walk-forward folds keep train and test dates disjoint, select parameters from training evidence only after minimum-trade, non-negative Sharpe, and Profit Factor gates, report frozen test metrics without promoting a losing candidate, and retain rejected-candidate audit counts and metrics.
...
- The historical-orb-backtest CLI validates offline by default and requires --run for research and create-only report output; it never loads the Moomoo SDK or accesses an account or order API.
- Tests cover every lifecycle exit, stop-first ambiguity, no-lookahead entry, costs, attribution, cache tampering, artifact escape, walk-forward isolation, long-only behavior, report conflicts, and no-broker CLI behavior.
- English, Japanese, Simplified Chinese, CLI, operations, limitations, rollback, feature documentation, implementation plan, ignore rules, and generated Task Catalog output are synchronized.
