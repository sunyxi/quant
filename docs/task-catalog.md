...
- Roadmap: see `docs/roadmap.md#phase-4`
- Summary: Change the broker integration priority so the first real API proof of concept uses Moomoo OpenAPI on macOS for sanitized read-only US capability discovery, while retaining kabu Station for later JP cash-equity execution and IBKR as a US fallback.
...
- Japanese market-data access is documented as entitlement-dependent and must not be assumed.
...
- Roadmap: see `docs/roadmap.md#phase-4`
- Summary: Add a macOS-capable read-only Moomoo OpenD discovery boundary with an injected fake SDK for tests and sanitized compatibility evidence without placing paper or live orders.

### Acceptance Criteria

- A broker-specific boundary checks local OpenD reachability without coupling strategy code to the Moomoo SDK.
- The discovery boundary reads only API version, account-list shape, US market capability, market-data entitlement metadata, and paper-account availability needed for compatibility decisions.
